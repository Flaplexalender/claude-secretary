...
- [x] **Dead-binding filter in `self_improve.py`** (commit `1d9c955`) — added `_find_dead_literal_bindings` + `_reject_dead_binding_changes` that AST-scan promoted .py changes, flag newly-added top-level literal-valued constants with zero references anywhere in sandbox `src/` or `tests/`, and reject the promotion when ≥80% of added bytes are flag-shaped. Mixed files (real edits + some dead bindings) are kept with a `HealthLog` warning. Strengthened `_CHANGE_PLAN_SYSTEM` prompt with explicit anti-cargo-cult rule. 26 unit tests in `tests/test_self_improve_dead_bindings.py`. CI green on run 24695473518. Structurally prevents recurrence of the 2026-04-20 regression.
